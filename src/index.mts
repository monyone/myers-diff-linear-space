  ...
  for (let depth = 0; depth <= max_depth; depth++) {
    for (let direction = 0; direction < 2; direction++) {
      ...
      for (let k = from_k; k <= to_k; k += 2) {
        ...
        const is_opposite_k_in_depth = opposite_k >= -(depth - (1 - direction)) && opposite_k <= (depth - (1 - direction));
        // (is_forward && total_length % 2 !== 0) || (!is_forward && total_length % 2 === 0)
        const parity = (total_length % 2) === (1 - direction);
        const unknown_condition = current[mod(k, compute_length)] + opposite[mod(opposite_k, compute_length)] >= from_length;
        // is_forward ? is_odd_total_length : !is_odd_total_length;
        if (parity && is_opposite_k_in_depth && unknown_condition) {
